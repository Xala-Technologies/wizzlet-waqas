...
import { dashboardSidebarAsideClassName, DASHBOARD_CONTENT_CLASS, DASHBOARD_GUTTER_CLASS } from '@/lib/dashboardSidebar';
    ...
    <aside className={dashboardSidebarAsideClassName(mobile)}>
      {!mobile && (
        <div className="border-b border-border px-5 pb-5 pt-6">
          <button type="button" onClick={() => go('overview')} className="inline-flex">
            <SweephLogo size="sidebar" linkTo="" variant="light" />
          </button>
        </div>
      )}
        ...
        {sidebarItems.map(item => {
          ...
          return (
            <button
              key={item.label}
              onClick={() => go(item.tab)}
              className={`group flex h-12 w-full items-center gap-3.5 rounded-[var(--radius-md)] px-4 text-[15px] font-medium transition-colors duration-150 ${
                isActive
                  ? 'bg-[var(--active-bg)] font-semibold text-[var(--active-text)]'
                  : 'text-muted-foreground hover:bg-muted/70 hover:text-foreground'
              }`}
            >
              <item.icon className={`h-5 w-5 shrink-0 ${isActive ? 'text-[var(--active-text)]' : 'text-muted-foreground'}`} />
              {item.label}
