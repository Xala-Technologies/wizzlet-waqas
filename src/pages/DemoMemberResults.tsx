    ...
    <DemoMemberShell
      ...
      actions={
        <div className="flex gap-2">
          <Button size="sm" variant="outline" className="text-caption" onClick={exportCsv}><Download className="mr-1.5 h-3.5 w-3.5" /> Export</Button>
          <Button size="sm" className="text-caption" onClick={() => setOpen(true)}><Plus className="mr-1.5 h-3.5 w-3.5" /> Add pick</Button>
        </div>
      }
    >
      {/* Win rate hero */}
      <div className="rounded-xl border border-primary/20 bg-card p-6 mb-6">
        <div className="flex flex-wrap items-center gap-6">
          ...
          <div>
            ...
            <p className="text-caption text-muted-foreground mb-2">Based on {metrics.settledPicks} settled picks ({metrics.pending} pending)</p>
            <div className="flex flex-wrap items-center gap-3">
              <Badge variant="outline" className={`text-caption ${metrics.unitsNet >= 0 ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-destructive/10 text-destructive border-destructive/20'}`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={stat.label} className="rounded-xl border border-border bg-card p-4">
            ...
            {stat.sub && <p className="text-caption text-primary mt-0.5 truncate">{stat.sub}</p>}
          </div>
        ))}
      </div>
      ...
      {weekly.length > 0 && (
        <div className="rounded-xl border border-border bg-card p-5 mb-6">
          <h2 className="text-caption font-semibold uppercase tracking-wider text-muted-foreground mb-4">Weekly Breakdown</h2>
          <div className="space-y-3">
            {weekly.map(w => (
              <div key={w.week} className="flex items-center gap-4">
                <span className="text-caption text-muted-foreground w-16">{w.week}</span>
                <div className="flex-1"><Progress value={(w.wins / w.plays) * 100} className="h-2" /></div>
                <span className="text-caption font-medium w-24 text-right">{w.wins}/{w.plays} ({Math.round((w.wins / w.plays) * 100)}%)</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {metrics.byCreator.length > 0 && (
        <div className="rounded-xl border border-border bg-card p-5 mb-6">
          <h2 className="text-caption font-semibold uppercase tracking-wider text-muted-foreground mb-4">Performance by Creator</h2>
          <div className="space-y-2">
            {metrics.byCreator.map(c => (
              <div key={c.creator.id} className="flex items-center gap-3 text-caption">
          ...
          {rows.map((t, i) => {
            ...
            return (
              <div key={t.id} className={`flex flex-wrap items-center gap-3 px-5 py-3.5 ${i < rows.length - 1 ? 'border-b border-border' : ''} hover:bg-muted/20 transition-colors`}>
                ...
                <span className="text-caption text-muted-foreground">{format(new Date(t.date), 'MMM d')}</span>
                <span className={`text-caption font-medium w-14 text-right ${pl > 0 ? 'text-emerald-500' : pl < 0 ? 'text-destructive' : 'text-muted-foreground'}`}>
                  {pl > 0 ? '+' : ''}{pl}u
                </span>
                {t.result === 'pending' ? (
                  <div className="flex gap-1">
                    {(['won', 'lost', 'push'] as PickResult[]).map(r => (
                      <Button key={r} size="sm" variant="outline" className="h-6 px-2 text-caption capitalize"
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-3">
            <div>
              <Label className="text-caption">Event</Label>
            ...
            <div>
              ...
              <p className="text-caption text-muted-foreground mt-1">EU odds: {usToDecimal(form.usOdds)?.toFixed(2) ?? '—'}</p>
            </div>
          </div>
          <DialogFooter>
