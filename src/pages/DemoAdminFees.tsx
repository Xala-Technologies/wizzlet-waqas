  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="rounded-xl border border-border bg-card p-5"><DollarSign className="h-4 w-4 text-primary mb-2" /><p className="text-2xl font-bold">${metrics.volume.toLocaleString()}</p><p className="text-caption text-muted-foreground mt-1">Total Platform Volume</p></div>
        <div className="rounded-xl border border-border bg-card p-5"><Percent className="h-4 w-4 text-primary mb-2" /><p className="text-2xl font-bold">${metrics.fees.toLocaleString()}</p><p className="text-caption text-muted-foreground mt-1">Platform Earnings ({metrics.effectiveFeeRate}% effective)</p></div>
        <div className="rounded-xl border border-border bg-card p-5"><TrendingUp className="h-4 w-4 text-primary mb-2" /><p className="text-2xl font-bold">${metrics.payouts.toLocaleString()}</p><p className="text-caption text-muted-foreground mt-1">Creator Payouts</p></div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6 mb-6">
        ...
        <div className="grid sm:grid-cols-3 gap-4 max-w-2xl">
          <div>
            <Label htmlFor="fee-standard" className="text-caption">Standard fee (%)</Label>
          ...
          <div>
            <Label htmlFor="fee-intro" className="text-caption">Intro fee (%)</Label>
            <Input id="fee-intro" type="number" min="0" max="50" step="0.5" value={intro} onChange={e => setIntro(e.target.value)} className="mt-1" />
          </div>
          <div>
            <Label htmlFor="fee-days" className="text-caption">Intro period (days)</Label>
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          ...
          {dirty && (
            <Button size="sm" variant="ghost" className="text-caption" onClick={discard}>Discard changes</Button>
          )}
          {!dirty && (
            <Button size="sm" variant="ghost" className="text-caption text-muted-foreground" onClick={restoreDefaults}>Restore defaults</Button>
          )}
            ...
            <div key={cf.creator.id} className="flex items-center justify-between gap-4 rounded-lg px-3 py-2.5 hover:bg-muted/20 transition-colors">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div>
                  ...
                  <p className="text-caption text-muted-foreground">{cf.creator.subs} subscribers · {cf.transactions} transactions · ${cf.volume.toLocaleString()} volume</p>
                </div>
              </div>
