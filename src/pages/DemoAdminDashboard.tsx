    ...
    <>
      <div className="mb-8 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="outline" className="text-caption" onClick={exportSummary}>
            <Download className="mr-1.5 h-3.5 w-3.5" /> Export summary
          </Button>
          <Button size="sm" variant="ghost" className="text-caption" onClick={reset}>
      ...
      {state.applications.length > 0 && (
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 mb-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <Button size="sm" variant="ghost" asChild className="text-caption">
            ...
            {state.applications.slice(0, 3).map(app => (
              <div key={app.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-border bg-card px-3 py-2.5">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-caption text-muted-foreground truncate">{app.pitch}</p>
                </div>
                <span className="text-caption text-muted-foreground shrink-0">{app.appliedDaysAgo}d ago</span>
                <div className="flex gap-2 shrink-0">
                  <Button size="sm" className="h-7 text-caption" onClick={() => approveApplication(app.id)}>Approve</Button>
                  <Button size="sm" variant="outline" className="h-7 text-caption" onClick={() => rejectApplication(app.id)}>Reject</Button>
                </div>
              </div>
            ))}
          </div>
      ...
      {inactiveCreators.length > 0 && (
        <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-4 mb-6 flex items-center gap-3">
          ...
          <Button size="sm" variant="outline" asChild className="text-caption"><Link to="/demo/admin/creators">Manage</Link></Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        {stats.map(stat => (
          <Link key={stat.label} to={stat.to} className="rounded-xl border border-border bg-card p-5 transition-colors hover:border-primary/30">
            ...
            <p className="text-caption text-muted-foreground/70 mt-0.5">{stat.sub}</p>
          </Link>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-2">Month to date</p>
          <p className="text-3xl font-bold">{money(thisMonth?.revenue ?? 0)}</p>
          <p className={`text-caption mt-1 inline-flex items-center gap-1 ${growthUp ? 'text-primary' : 'text-destructive'}`}>
            {growthUp ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
            {growthUp ? '+' : ''}{growth}% vs {lastMonth?.month ?? 'last month'}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-2">Refunded volume</p>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-2">Fee tiers in force</p>
          <p className="text-3xl font-bold">{state.settings.introFeePercent}% → {state.settings.standardFeePercent}%</p>
          <p className="text-caption text-muted-foreground mt-1">intro for first {state.settings.introPeriodDays} days</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/demo/admin/fees" className="text-caption text-primary hover:underline">Fee breakdown</Link>
        </div>
        <div className="h-64">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-2">Active Subscriptions</p>
          <p className="text-3xl font-bold">{metrics.activeTransactions}</p>
          <p className="text-caption text-muted-foreground mt-1">across {metrics.activeCreators} active creators</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-2">Avg Volume / Creator</p>
          <p className="text-3xl font-bold">{money(metrics.avgRevenuePerCreator)}</p>
          <p className="text-caption text-muted-foreground mt-1 inline-flex items-center gap-1">
            <Percent className="h-3 w-3" /> effective fee rate {metrics.effectiveFeeRate}%
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-2">Creator Payouts Owed</p>
          <p className="text-3xl font-bold">{money(metrics.payouts)}</p>
          <p className="text-caption text-muted-foreground mt-1">min payout ${state.settings.minPayoutAmount}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/demo/admin/fees" className="text-caption text-primary hover:underline">All</Link>
          </div>
              ...
              {topCreators.map((c, i) => (
                <div key={c.creator.id} className="flex items-center justify-between gap-4 rounded-lg px-3 py-2.5 hover:bg-muted/20 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="w-5 text-caption text-muted-foreground tabular-nums">{i + 1}</span>
                    <div className="min-w-0">
                      <p className="text-sm font-medium truncate">
                        {c.creator.name}
                        {!c.creator.active && <span className="ml-2 text-caption text-destructive">disabled</span>}
                      </p>
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/demo/admin/transactions" className="text-caption text-primary hover:underline">All transactions</Link>
          </div>
          {recent.length === 0 ? (
              ...
              {recent.map(a => (
                <div key={a.id} className="flex items-center justify-between gap-4 rounded-lg px-3 py-2.5 hover:bg-muted/20 transition-colors">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`rounded-full px-2 py-0.5 text-caption font-medium capitalize ${statusStyles[a.status]}`}>{a.status}</span>
                    <span className="text-sm font-medium">{money(a.amount)}</span>
                    <span className="text-caption text-muted-foreground w-12 text-right">{format(a.date, 'MMM d')}</span>
                  </div>
                </div>
              ))}
            </div>
