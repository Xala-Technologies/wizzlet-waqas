    ...
    <DashboardLayout type="admin">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-8">
        ...
        <div className="flex gap-2">
          <Button asChild variant="outline" size="sm" className="h-9 text-caption"><Link to="/admin/payouts">Payouts</Link></Button>
          <Button asChild variant="outline" size="sm" className="h-9 text-caption"><Link to="/admin/reports">Export</Link></Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <p className="text-caption text-muted-foreground mt-1">Gross Revenue (all time)</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <Percent className="h-4 w-4 text-purple-400 mb-2" />
          <p className="text-2xl font-bold text-emerald-400">{fmt(stats.feeRevenue)}</p>
          <p className="text-caption text-muted-foreground mt-1">Platform Fee Revenue · {stats.effectiveRate.toFixed(1)}% effective</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <TrendingUp className="h-4 w-4 text-blue-400 mb-2" />
          <p className="text-2xl font-bold">{fmt(stats.mrr)}</p>
          <p className="text-caption text-muted-foreground mt-1">MRR · {stats.activeCount} active subs</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <Wallet className="h-4 w-4 text-amber-400 mb-2" />
          <p className="text-2xl font-bold text-amber-400">{fmt(stats.liability)}</p>
          <p className="text-caption text-muted-foreground mt-1">Unpaid Creator Liability</p>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-1">Fee MRR</p>
          <p className="text-xl font-bold text-emerald-400">{fmt(stats.feeMrr)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-1">Creator Earnings</p>
          <p className="text-xl font-bold">{fmt(stats.creatorEarnings)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-1">Paid Out</p>
          <p className="text-xl font-bold">{fmt(stats.paidOut)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-caption text-muted-foreground uppercase tracking-wider mb-1">Payouts In Progress</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {stats.topCreators.map((c, i) => (
                <div key={c.id} className="flex items-center justify-between gap-4 px-4 py-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="text-caption text-muted-foreground w-4">{i + 1}</span>
                    <div className="min-w-0">
                      <p className="text-sm font-medium truncate">{c.name}</p>
                      <p className="text-caption text-muted-foreground">{c.subs} active · {fmt(c.fees)} fees</p>
                    </div>
                  </div>
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="p-4 border-b border-border bg-muted/30 flex items-center justify-between">
            ...
            <Link to="/admin/transactions" className="text-caption text-primary inline-flex items-center gap-1">All <ArrowUpRight className="h-3 w-3" /></Link>
          </div>
              ...
              {stats.recentTransactions.map((s) => (
                <div key={s.id} className="flex items-center justify-between gap-4 px-4 py-3">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <Badge variant="outline" className={`text-caption ${s.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-muted text-muted-foreground'}`}>
                      {s.status}
                    </Badge>
                  </div>
                </div>
