              ...
              return (
                <article key={post.id} className="rounded-xl border border-border bg-card p-5">
                  <div className="flex items-center gap-2.5 mb-2">
                    ...
                    <span className="text-caption text-muted-foreground">{format(new Date(post.createdAt), 'MMM d')}</span>
                  ...
                  <p className={`text-caption text-muted-foreground line-clamp-2 mb-3 ${locked ? 'blur-[3px] select-none' : ''}`}>
                    {locked ? post.preview : post.content}
                  </p>
                  <div className="flex items-center gap-3">
                    <button onClick={() => store.toggleLike(post.id)} className={`flex items-center gap-1 text-caption transition-colors hover:text-primary ${liked ? 'text-primary' : 'text-muted-foreground'}`}>
                      <Heart className={`h-3.5 w-3.5 ${liked ? 'fill-current' : ''}`} /> {post.likes}
                    </button>
                    <button onClick={() => { store.toggleSave(post.id); toast.info('Removed from saved'); }} className="flex items-center gap-1 py-2 -my-2 text-caption text-muted-foreground hover:text-destructive transition-colors">
          ...
          {bookmarked.map(c => {
            ...
            return (
              <div key={c.id} className="rounded-xl border border-border bg-card p-5">
                ...
                <p className="text-caption text-muted-foreground mb-3">{c.bio}</p>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-primary">${c.price}/mo</span>
                  {subscribed ? (
                    <CancelSubButton creatorId={c.id} />
                  ) : (
                    <Button size="sm" className="h-7 text-caption" onClick={() => { store.subscribe(c.id); toast.success(`Subscribed to ${c.name}`); }}>
