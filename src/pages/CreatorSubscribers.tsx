...
const CreatorSubscribers = () => {
  ...
  const statusBadge = (status: string) =>
    status === 'active' ? (
      <span className="inline-flex items-center gap-1 text-caption font-medium text-emerald-400">
              ...
              <li key={sub.id} className="rounded-xl border border-border bg-card p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-caption text-muted-foreground truncate mt-0.5">{sub.user?.email}</p>
                  </div>
            ...
            <table className="w-full min-w-[520px]">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  <th className="text-left text-caption font-medium text-muted-foreground p-4">Subscriber</th>
                ...
                {subscribers.map((sub) => (
                  <tr key={sub.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                    <td className="p-4">
                      ...
                      <p className="text-caption text-muted-foreground">{sub.user?.email}</p>
                    </td>
                    <td className="p-4">{statusBadge(sub.status)}</td>
                    <td className="p-4 text-caption text-muted-foreground">{format(new Date(sub.created_at), 'MMM d, yyyy')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
